...
export class peerJsServer  implements Server{
    readonly events: TypedEventEmitter<ServerEvents> = new TypedEventEmitter<ServerEvents>();
    
    public id: string = "";
    protected peer: Peer;
    protected clients:  Map<number,PeerJsClientData> = new Map<number,PeerJsClientData>();
    ...
    constructor(){
        ...
        this.peer.on('connection',(dataConnection)=>{
            ...
            const newClient = new PeerJsClientData(newId,dataConnection.metadata as string,dataConnection.peer,dataConnection.connectionId);
            console.log("Client connected!", newId , dataConnection.connectionId, newClient.username);
    ...
    sendMessageAllExcept(message: Message, ... id: number[]): void {
        this.clients.forEach(client => {
            if(id.some(x=>x===client.id) ) 
                return;
            const connection = this.getDataConnection(client.id);
            if(connection == null)
                throw RangeError(`connection for client ${client.id} does not exist`);
            connection.send(message)
        });
    }
...
class PeerJsClientData implements ClientData {
    constructor (id: number, username: string, peerId: string, connectionId: string){
        this.id = id;
